...
def load_vectorstore():
    ...
    if not os.path.exists(persist_dir):
        raise ValueError("dementiaDB folder not found. Make sure it is in the project root.")
...
def load_vectorstore():
    ...
    # If DB already exists → load fast
...
def rag_answer(user_message: str, history=None):
    ...
    try:
        # Build a combined conversation string for better context
        full_question = ""
        ...
        # Send the combined conversation to the RAG chain
        return qa_chain.run(full_question)
